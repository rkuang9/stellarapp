...
import { MetricsLabels, Metrics, Optimizers, Metric, Loss, Optimizer, LossesSegmentation } from '@/types/hyperparameters';
...
export default function Hyperparameters() {
    ...
    const field_errors: { [key: string]: string | undefined } = {
        ...
        loss_function: !!(LossesSegmentation as any)[project_config.model.loss_fn as Loss] ? undefined : "Choose a loss function",
    ...
    const onChangeBatchSize = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isInt(value)) {
            project_config.model.batch_size = clamp(0, Number(value), 1024);
            render();
        }
    }
        ...
        <div className="md:grid grid-cols-12 flex flex-col overflow-auto gap-y-2 grow w-full md:max-w-2xl py-1">
            ...
            <div className={field_layout_css}>
                <div className="grow">
                    <SelectField
                        ...
                        options={{
                            [Loss.DICE_BINARY_CROSS_ENTROPY]: LossesSegmentation[Loss.DICE_BINARY_CROSS_ENTROPY],
                            [Loss.DICE_CATEGORICAL_CROSS_ENTROPY]: LossesSegmentation[Loss.DICE_CATEGORICAL_CROSS_ENTROPY],
                            [Loss.CATEGORICAL_CROSS_ENTROPY]: LossesSegmentation[Loss.CATEGORICAL_CROSS_ENTROPY],
                            [Loss.BINARY_CROSS_ENTROPY]: LossesSegmentation[Loss.BINARY_CROSS_ENTROPY],
                        }}
            ...
            <div className={field_layout_css}>
                <div className="grow">
                    ...
                    >
                        {[Metric.ACCURACY, Metric.PRECISION, Metric.RECALL].map(metric => <MultiSelectOption
