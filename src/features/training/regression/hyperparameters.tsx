...
import { Losses, MetricsLabels, Metrics, Optimizers, Loss, Optimizer, Metric } from '@/types/hyperparameters';
...
export const val_split_options: { [key: string]: string } = {
    "0.4": "Training 60% · Validation 40%",
    "0.3": "Training 70% · Validation 30%",
    "0.2": "Training 80% · Validation 20%",
    "0.1": "Training 90% · Validation 10%",
    "0": "Training 100% · Validation 0%",
};
export const field_layout_css = "col-span-12 sm:odd:col-span-5 sm:even:col-span-7 lg:odd:col-span-4 lg:even:col-span-8 flex items-center gap-2 grow basis-0";
export const error_text_css = "text-error text-sm";


export default function Hyperparameters() {
    const { status, dataframe } = React.useContext(RegressionContext);
    const { project_config, full_render, worker, cache } = useProject<ProjectConfig, RegressionModelWorker>();
    ...
    React.useEffect(() => {
        ...
        if (project_config.model.loss_fn == "sparseCategoricalCrossentropy" &&
            (project_config.preprocessing.target_cols.length != 1 ||
                dataframe.types[project_config.preprocessing.target_cols[0]] != "string")) {
            project_config.model.loss_fn = "";
            full_render();
        }
    }, [project_config.preprocessing.target_cols, dataframe, project_config.model.loss_fn])
            ...
            <div className={field_layout_css}>
                <div className="grow">
                    <MultiSelect
                        ...
                        disabled={worker?.isTraining()}
                        className={project_config.model.metrics.length == 0 ? "text-muted-foreground hover:text-muted-foreground" : ""}
                    ...
                    >
                        {Object.keys(Metrics).map(metric => <MultiSelectOption
                            ...
                            //onMouseUp={() => onChangeMetrics(metric)}
                            key={metric}
                        >
                            {Metrics[metric]}
                        </MultiSelectOption>)}
...
export function FieldLabel({ title, description, notify, error, disabled }: {
    title: string,
    description: string;
    ...
    return <Label className="flex flex-col">
        <div className={`flex gap-2 ${disabled ? "text-muted-foreground" : ""}`}>
            {title}
            <Info
        ...
        {error && <span className="invisible">error</span>}
    </Label>
}
