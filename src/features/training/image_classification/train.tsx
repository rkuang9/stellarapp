...
import { OneHotEncodingArgs, MinMaxScaleArgs, ZScoreArgs, InputShapeArgs } from "@/lib/data-processing/project_config";
...
export default function ModelTrain() {
    ...
    const trainStart = async () => {
        if (!(worker instanceof ImageModelWorker)) {
            await notify({
        ...
        try {
            if (!status.dataset || !status.layers || !status.hyperparameters) {
                notify({
            ...
            } = createDummyImageClassificationDataset(project_config);
            ...
            if (cache.cloud_model_state == "can_download" && !worker.isBuilt() && mode == "resume" && downloadModel) {
                if (await confirmation({
                    ...
                    try {
                        ...
                        logError({
                            source: "training/image_classification/train",
            ...
            (worker as ImageModelWorker)?.train({
                project_config,
                mode: `${mode}_train`,
                dataset: await imageset.arrayBuffer(),
            }).then(() => {
...
function createDummyImageClassificationDataset(project_config: ProjectConfig): {
    xs: number[][];
    ys: number[][];
    ...
    // not calling preprocess_image.createOneHotEncoding because that causes
    // tfjs to re-register the backend for various ops (terminal spam),
    // and creating onehot encoding doesn't even need anything from tfjs,
    // just note that for generating new onehot encoding in image classification,
    // categories are always sorted, but existing encodings' order is respected
    const categories = project_config.preprocessing.input_cols;

    const onehot_encoding = Object.fromEntries(
        categories.toSorted().map((category, index) => [category, index]));

    return { xs: [], ys: [], input_shape: project_config.model.input_shape, onehot_encoding, scale_cols: {}, zscore_cols: {} }
}
