...
import { SegmentationPredictArgs, WorkerState, WorkerTrainArgs } from "@/lib/webworker/worker_types";
import BaseModelWorker from "@/lib/webworker/base_client";


export default class SegmentationModelWorker extends BaseModelWorker {

    constructor(project_config: ProjectConfig) {
        super(project_config, new Worker(new URL("@/lib/webworker/segmentation_worker", import.meta.url)));
    }


    public override async predict({ inputs, backend, batch_size, depth, colorize = true }: Omit<SegmentationPredictArgs, "state">): Promise<File> {
        const promise = this.registerListener("predict");

        const predict_data: SegmentationPredictArgs = {
            state: WorkerState.PREDICT, inputs, backend, batch_size, depth, colorize
        }
    ...
    public override reset(project_config: ProjectConfig): void {
        super.reset(project_config, new Worker(new URL("@/lib/webworker/segmentation_worker", import.meta.url)));
    }
}
