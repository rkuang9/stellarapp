...
import { WorkerPredictArgs, WorkerState, WorkerTrainArgs } from "@/lib/webworker/worker_types";
import BaseModelWorker from "@/lib/webworker/base_client";
import { TrainingBackend } from "@/types/hyperparameters";


export default class RegressionModelWorker extends BaseModelWorker {

    constructor(project_config: ProjectConfig) {
        super(project_config, new Worker(new URL("@/lib/webworker/regression_worker", import.meta.url)));
    }


    public override async predict({ inputs, backend, batch_size }: {
        inputs: number[][];
        ...
        const predict_data: WorkerPredictArgs = {
            state: WorkerState.PREDICT, inputs, backend, batch_size
        }
    ...
    public override reset(project_config: ProjectConfig): void {
        super.reset(project_config, new Worker(new URL("@/lib/webworker/regression_worker", import.meta.url)));
    }
}
